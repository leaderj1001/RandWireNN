...
from dropPath_model import DropModel
from preprocess import load_data
from plot import draw_plot
from visualize import layer_visualize
...
def main():
    ...
    parser.add_argument('--c', type=int, default=109, help='channel count for each node, (example: 78, 109, 154), (default: 109)')
    ...
    name = "c_" + str(args.c) + "_p_" + str(args.p) + "_graph_mode_" + args.graph_mode
    if args.load_model:
        if args.drop_path > 0:
            model = DropModel(args.node_num, args.p, args.c, args.c, args.graph_mode, args.model_mode, args.dataset_mode, args.is_train, name, args.drop_path).to(device)
        else:
            model = Model(args.node_num, args.p, args.c, args.c, args.graph_mode, args.model_mode, args.dataset_mode, args.is_train, name).to(device)
        ...
        if args.drop_path > 0:
            model = DropModel(args.node_num, args.p, args.c, args.c, args.graph_mode, args.model_mode,
                              args.dataset_mode, args.is_train, name, args.drop_path).to(device)
        else:
